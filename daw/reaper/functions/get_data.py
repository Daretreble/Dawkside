			...
			def zone_load(hist):
				if len(hist['tr']) > 0:
					for key,value in hist['tr'].items():
						for _ in self.online['keys']:
							if key == _.short_name:
								_.zones_config = value
				else:
					for _ in self.online['keys']:
						_.zone_manage(0,action='zonepreset',speak=False)
		...
		if 'fx' in args[0]:
			...
			if reapy_track.n_fxs > 0:
				...
				if 'hist' in locals() and len(hist['tr']) == 0 and plugins.user.is_saved('plugin') and 'zones' in plugins.user_params[plugins.name] and plugins.subname in plugins.user_params[plugins.name]['zones']:
					for key,value in plugins.user_params[plugins.name]['zones'][v.subname].items():
						for __ in self.online['keys']:
							if __.short_name == key:
								self.track.history[self.track.reapy_track.id]['tr'][__.short_name] = copy.deepcopy(value)
								__.zoneconf = value
				else:
					if 'hist' in locals():
						zone_load(hist)
