...
def zone_manage(self,*args,**kwargs):
	...
	if action == 'pluginzoneclear':
		...
		if modif('test',[911]):
			...
			if Plugins.act:
				...
				if Plugins.isSaved('plugin'):
				
					Plugins.userParams[Plugins.name]['zones'] = {}
					Plugins.upSave()
	...
	if action == 'pluginzonesave':
		...
		if modif('test',[911]):
			...
			if Plugins.act:
				...
				if Plugins.isSaved('plugin') == False:
				
					Plugins.upCreate()
			
				tmp = Plugins.userParams[Plugins.name]['zones']
	...
	if action == 'trackzonesave':
		...
		if modif('test',[911]):
			...
			zonetmp = copy.deepcopy(self.zones_config)
			tmp = self.daw.track.history[self.daw.track.reapy_track.id]['tr']
	...
	if action == 'split':
		...
		if modif('test',[911]):
			...
			pprint.pprint(self.zones_config)
